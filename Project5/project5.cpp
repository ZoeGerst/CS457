...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Zoe Gerst" };
...
Display( )
{
	...
	Magic->Use( );
	Magic->SetUniformVariable( "uKa", 0.1f );
	Magic->SetUniformVariable( "uKd", 0.6f );
	Magic->SetUniformVariable( "uKs", 0.3f );
	Magic->SetUniformVariable( "uShininess", 8.f );

	Magic->SetUniformVariable( "uS0", S0);
	Magic->SetUniformVariable( "uT0", T0 );
	Magic->SetUniformVariable( "uD", D*(float)(.5+.5*sin(2.*M_PI*Time)) );
	Magic->SetUniformVariable( "uTime",  Time );
...
InitGraphics( )
{
	...
	Magic = new GLSLProgram( );
	bool valid = Magic->Create( "magic.vert",  "magic.frag" );
	if( ! valid )
	{
