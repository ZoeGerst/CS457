...
Display( )
{
	...
	OvalNoise->Use( );
	OvalNoise->SetUniformVariable( "uKa", 0.1f );
	OvalNoise->SetUniformVariable( "uKd", 0.6f );
	OvalNoise->SetUniformVariable( "uKs", 0.3f );
	OvalNoise->SetUniformVariable( "uShininess", 8.f );

	OvalNoise->SetUniformVariable( "uS0", S0);
	OvalNoise->SetUniformVariable( "uT0", T0 );
	OvalNoise->SetUniformVariable( "uD", D*(float)(.5+.5*sin(2.*M_PI*Time)) );
	OvalNoise->SetUniformVariable( "uTime",  Time );
...
InitGraphics( )
{
	...
	OvalNoise = new GLSLProgram( );
	bool valid = OvalNoise->Create( "ovalnoise.vert",  "ovalnoise.frag" );
	if( ! valid )
	{
